 ...
 * et ajoute des méthodes de recherche par email et par ID avec chargement
 * des associations nécessaires.
 *
 * @param <T> le type d’utilisateur géré (ex. Admin, Carrier, Exporter, …)
 */
@NoRepositoryBean
public interface GenericUserRepository<T extends User> extends JpaRepository<T, Integer> {
	 ...
	 * son association {@code language} pour éviter le lazy loading ultérieur.
	 *
	 * @param id l’identifiant numérique de l’utilisateur
	 * @return un {@link Optional} contenant l’utilisateur avec ses associations, ou vide si non trouvé
	 */
