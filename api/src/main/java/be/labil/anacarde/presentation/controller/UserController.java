...
public class UserController {
     ...
     */
    @Operation(summary = "Get user details", description = "Returns the detailed information of a user by its ID.")
    @ApiResponses({
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "User created successfully",
                    content = @Content(schema = @Schema(implementation = UserDto.class))),
            @ApiResponse(responseCode = "400", description = "Validation error or invalid JSON", content = @Content)
    })
    @PostMapping(consumes = "application/json")
    public ResponseEntity<UserDto> createUser(
            @Validated({Default.class, ValidationGroups.Create.class}) @RequestBody UserDto userDto) {
     ...
     */
    @Operation(summary = "Update a user", description = "Updates an existing user using the ID provided in the URL.")
    @ApiResponses({
    ...
    public ResponseEntity<UserDto> updateUser(
            @Parameter(description = "User ID", example = "1", required = true)
            @PathVariable("id") Integer id,
            @Validated({Default.class, ValidationGroups.Update.class}) @RequestBody UserDto userDto) {
