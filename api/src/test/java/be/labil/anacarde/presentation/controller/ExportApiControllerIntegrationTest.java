...
class ExportApiControllerIntegrationTest extends AbstractIntegrationTest {
	...
	void listAuctions_between_two_dates() throws Exception {
		...
		var start = ref.getAuctionStartDate().minusMinutes(1)
				.format(DateTimeFormatter.ISO_DATE_TIME);
		var end = ref.getAuctionStartDate().plusMinutes(1).format(DateTimeFormatter.ISO_DATE_TIME);

		mockMvc.perform(get("/api/export/auctions").param("start", start).param("end", end)
				.param("onlyEnded", "false").with(user(getProducerTestUser()))
				.accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$").isArray())
				.andExpect(jsonPath("$[0].auctionId").value(ref.getAuctionId()));
	}
}
