...
public abstract class AbstractIntegrationTest {
    ...
    public void initUserDatabase() {
        ...
        User user1 = User.builder().firstName("John").lastName("Doe").email("user@example.com").password("$2a$10$abcdefghijklmnopqrstuv1234567890AB").registrationDate(LocalDateTime.now()).active(true).build();
        User user2 = User.builder().firstName("Foo").lastName("Bar").email("foo@bar.com").password("$2a$10$abcdefghijklmnopqrstuv1234567890AB").registrationDate(LocalDateTime.now()).active(true).build();
